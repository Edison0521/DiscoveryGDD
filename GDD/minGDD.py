...
def combinetitle(title,df):
    ...
    # .appName("Distributed Data Analysis") \
    # .getOrCreate()
    #    .master("spark://12.12.12.100:7077") \
    # 将 g 字典转换为 RDD
    #g_rdd = spark.sparkContext.parallelize([(k, v) for k, v in g.items()])

    # 使用 flatMap 对 RDD 应用 process_item 函数
    #processed_rdd = g_rdd.flatMap(lambda item: process_item(item))

    # 使用 collect 收集处理结果并将其转换为列表
    #processed_list = processed_rdd.collect()
    ...
    #print(processed_list+vblock)
    # with open('t.txt', 'w') as f:
    #     for line in processed_list:
    #         f.write(str(line) + '\n')


    return patternblock+constblock+vblock+idblock
...
def process_item(item):
    ...
    if str(key) != 'id':
        for val in value:
            if len(value[val]) > 2:
                c = creatlist(value[val])
                sb = []
                for var1 in c:
                    s =  str(re.sub('[^a-zA-Z]+', '', str(value[val][0].name))+'.'+ str(value[val][0].attribute) + '=' + str(var1[0].id))
                    iteml = GDD_new.iteml(s,var1[0].name,var1[0].attribute,0)
                    if iteml not in sb:
                        sb.append(iteml)
                result.append(sb)
            elif len(value[val]) <= 2:
                r = relationbolck(value[val][0].lists)
                result.append(r)
    elif str(key) == 'id':
        for val in value:
            #result.append(val)
                                ...
                                if (set(lhsMatches).issubset(set(rhsMatches))):
                                    ...
                                    for i in range(len(lhsSet)):
                                        if (i == len(lhsSet) - 1):
                                            ...
                                            dependency += lhsSet[i].name + ' ' + str(lhsSet[i].sigma) + ";;"
                                    dependency += "->"
                                    dependency += rhs.name + ' ' + str(rhs.sigma) + ' support is '+str(len(lhsSet[0].l))+" "+ str(500/len(rhs.l))
                                    # print(dependency)
        ...
        for single_value in value:
            if len(sli) == 2:
                ...
                for i in range(0, 2):
                    ...
                    new_value1 = res_dict.get(new_str1, [])
                    new_value2 = res_dict.get(new_str2, [])
...
if __name__ == "__main__":
    ...
    #     filename = 'produce_Table0.txt'
    #     files = str(con)+'_'+str(seg)+'result/'+filename+str(con)+' '+str(seg)+' '+ 'npresultwd.txt'
    #     file = pd.read_csv(filename, delimiter=";;", engine='python')
    ...
    g = glob.glob('*.txt')
    for gi in g:
        #gi = 'produce_Table2.txt'
        con = float(sys.argv[1])
        seg = int(sys.argv[2])
        thresord = con
        
        num = int(sys.argv[3])
        filename = gi
        # os.mkdir(str(con)+'_'+str(seg)+'result')
        dir_name = str(thresord) + '_' + str(seg) + "_"+str(num) + 'result'
        if not os.path.exists(dir_name):
            os.mkdir(dir_name)
        files = str(thresord) + '_' + str(seg) + "_"+str(num) + 'result/'+filename+str(thresord)+'_'+str(seg) + " " + str(num) + ' '+ 'npresultwd.txt'
        ...
        title = title[:num]
        delete_keywords = []
        title = [df[cloum_name].name for cloum_name in df.columns if not any(keyword in cloum_name for keyword in delete_keywords)]
        # for cloum_name in df.columns:
        #     title.append(df[cloum_name].name)
