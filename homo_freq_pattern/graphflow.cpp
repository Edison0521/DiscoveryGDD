...
void Graphflow::FindMatches(uint order_index, uint depth, std::vector<uint> m, size_t &num_results)
{
    ...
    for (uint i = 0u; i < u_min_nbrs.size(); i++)
    {
        ...
        // {
        //     std::ofstream fout;
        //     fout.open("output1.txt", std::ios::app);           
        //     //std::cout<<'----------------------11111111111------------------------'<<std::endl;
        //     if (true)
        //     {
        //         // 检查 m 是否没有重复的元素
        //         std::set<uint> unique_elements(m.begin(), m.end());
        //         // if (unique_elements.size() == m.size())
        //         if (true)
        //         {
        //             num_results++;
        //             // 对 m 进行排序以确保节点顺序一致
        //             std::vector<uint> m_sorted(m.begin(), m.end());
        //             std::sort(m_sorted.begin(), m_sorted.end());
        //             // 将排序后的 m 转换为字符串以在集合中使用
        //             std::string m_str = "";
        //             for (const auto& elem : m_sorted)
        //             {
        //                 m_str += std::to_string(elem) + " ";
        //             }
        //             // 检查 m_str 是否已记录
        //             if (resultlist.find(m_str) == resultlist.end())
        //             {
        //                 // 记录 m_str 以避免重复记录
        //                 num_results--;
        //                 resultlist.insert(m_str);
        //                 fout << m_str << std::endl;
...
// void Graphflow::printResults(const std::vector<uint>& m, const std::string& filename) {
//     std::ofstream fout;
